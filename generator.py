...
def parser_salt_website_argument():
    ...
    parser.add_argument('-salt', '-s', type=str,
                        help='Add salt to mess up the encryption result.', default='')
    parser.add_argument('-url', '-u', type=str,
                        help='Treat the destination URL as an encrypted seed.', default='')
    return parser
...
if __name__ == "__main__":
    ...
    if args.salt == '':
        ...
        salt_each_lines.append(args.salt)

    if args.url == '':
        website_each_lines = read_from_file('web-site.txt', pw_source_folder)
    else:
        website_each_lines.append('{}:{},1'.format(args.url, args.url))

    f_output = open(os.path.join(
        os.getcwd(), pw_source_folder, 'output.txt'), 'w')
    for website_line in website_each_lines:
        website = website_line.split(":")[0].strip()
        url = website_line.split(':')[1].strip().split(',')[0].strip()
        salt_num = website_line.split(':')[1].strip().split(',')[1].strip()
        salt = salt_each_lines[int(salt_num)-1]
        hash_pw = hashlib.sha256(salt.encode(
            'utf-8') + url.encode('utf-8')).hexdigest()
    ...
    for i in range(0, len(website_pw_list), 2):
        print('{} : {}'.format(
            website_pw_list[i], website_pw_list[i + 1]), file=f_output)
    f_output.close()
